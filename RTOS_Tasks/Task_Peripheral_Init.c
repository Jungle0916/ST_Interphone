...
void Task_Peripheral_Init(void *argument)
{
    ...
    else
    {
        ...
        printf("BSP_SD_Init返回值: %d (MSD_OK=%d)\r\n", sd_result, MSD_OK);
        if(sd_result == MSD_OK)
        {
            printf("SD卡初始化成功！\r\n");
            ...
            {
                printf("文件系统挂载失败！错误码: %d\r\n", res);
                // 尝试格式化
                if(res == FR_NO_FILESYSTEM)
                {
                    printf("未找到文件系统，尝试格式化...\r\n");
                    // 使用较小的缓冲区，避免栈溢出
                    BYTE work[512];
